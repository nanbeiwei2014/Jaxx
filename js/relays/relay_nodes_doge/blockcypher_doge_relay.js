var DogeBlockcypherRelay = function() {
...
DogeBlockcypherRelay.prototype.initialize = function(relayManager) {
    this._relayManager = relayManager;
}

DogeBlockcypherRelay.prototype.getLastBlockHeight = function(){
	// This function shares a common interface with the other relays.
	return this._lastBlock;
}

DogeBlockcypherRelay.prototype.setLastBlockHeight = function(newHeight){
	this._lastBlock = newHeight;
}

DogeBlockcypherRelay.prototype.fetchLastBlockHeight = function(callback, passthroughParam) {
    ...
    RequestSerializer.getJSON(this._baseUrl+'v1/doge/main', function(response, status, passthroughParams) {
        if(status==='error'){
...
DogeBlockcypherRelay.prototype.checkCurrentHeightForAnomalies = function() {
...
DogeBlockcypherRelay.prototype.getTxList = function(addresses, callback) {
    this._relayManager.relayLog("Chain Relay :: " + this._name + " - Requested txlist for " + addresses);
    
    var self = this;
    RequestSerializer.getJSON(this._baseUrl+'v1/doge/main/addrs/' + addresses, function (response,status) {
...
DogeBlockcypherRelay.prototype.getTxListParse = function(primaryTxListData) {
    var txListItems = [];
...
DogeBlockcypherRelay.prototype.getTxCount = function(address, callback) {
    var self = this;
    
    this._relayManager.relayLog(this._name+" - Requested txCount for "+address);
    RequestSerializer.getJSON(this._baseUrl+'v1/doge/main/addrs/'+address, function (response,status) {
        var txCount = -1;
...
DogeBlockcypherRelay.prototype.getTxDetails = function(txHash, callback) {
    this._relayManager.relayLog("Chain Relay :: " + this._name+" - Requested tx details for "+txHash);
    console.log( this._baseUrl+'v1/doge/main/txs/'+txHash )
    
    var self = this;
    RequestSerializer.getJSON(this._baseUrl+'v1/doge/main/txs/'+txHash, function (response,status) {
...
DogeBlockcypherRelay.prototype.getTxDetailsParse = function(primaryTxDetailData) {
    var outputs = [],
        input = [];
...
DogeBlockcypherRelay.prototype.getAddressBalance = function(address, callback) {
...
DogeBlockcypherRelay.prototype.getFixedBlockHeight = function( address, callback ) {
    var self = this
...
DogeBlockcypherRelay.prototype.getUTXO = function(address, callback) {
    if (!callback) { throw new Error('missing callback'); }
    this._relayManager.relayLog("Chain Relay :: " + this._name+" - Requested UTXO for "+address);
    var url=this._baseUrl+'v1/doge/main/addrs/'+address+'?unspentOnly=true';
	var self = this;
...
DogeBlockcypherRelay.prototype.pushRawTx = function(hex, callback) {
...
//    });
	...
	var urlToCall = this._baseUrl + 'v1/doge/main/txs/push';
...
DogeBlockcypherRelay.prototype.getBlockcypherTxListCallback = function(response,prevTxs,callback) {
    ...
    if(response.txrefs) {
        ...
        if(response.hasMore){
            ...
            var newUrl = this._baseUrl+'v1/doge/main/addrs/'+address+'?limit=2000&before='+before; // (Dan) newUrl is the url for the next set of transactions.
...
DogeBlockcypherRelay.prototype.getRelayType = function() {
	return 'DogeBlockcypherRelay';
}

DogeBlockcypherRelay.prototype.getRelayTypeWithCallback = function(callback) {
	var relayName = 'DogeBlockcypherRelay';
	callback("success", relayName);
	return relayName; 
}

DogeBlockcypherRelay.prototype.getRelayTypeWithCallbackForgettingStatus = function(callback) {
	var relayName = 'DogeBlockcypherRelay';
	callback(relayName);
	return relayName; 
}
...
if (typeof(exports) !== 'undefined') {
    exports.relayBlockCypher = DogeBlockcypherRelay;
}