...
//    try {
    ...
    newTask.attachRelayManager(this);
...
//	try {
		...
		if (typeof(callbackArguments) !== 'undefined' && callbackArguments !== null) {  // Assures 'arguments' is defined
			if (typeof(callbackArguments[0]) !== 'undefined' && callbackArguments[0] !== null) {
				if (currentRelayTask.isCallbackSuccessful(callbackArguments[0], callbackArguments[1], callbackArguments[2])) { // Checks for a successful response from the serializer so that we know we can execute the callback.
...
//                        console.log("returning callbackArguments :: " + JSON.stringify(callbackArguments));
				...
				} else if (this._relayTasks[relayTaskIndex].isCallbackPermanentFailure(callbackArguments[0], callbackArguments[1], callbackArguments[2])) {
