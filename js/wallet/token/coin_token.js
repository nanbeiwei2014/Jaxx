...
CoinToken.Golem = 3;
CoinToken.Gnosis = 4;
CoinToken.Singulardtv = 5;
CoinToken.Digix = 6;
CoinToken.BlockchainCapital = 7;
CoinToken.Civic = 8;
CoinToken.numCoinTokens = 9;
...
CoinToken.getTokenCoinHolderType = function(tokenType) {
    ...
    if (tokenType === CoinToken.TheDAO) {
        ...
        return COIN_ETHEREUM;
    } else if (tokenType === CoinToken.Iconomi) {
        return COIN_ETHEREUM;
    }
    */
    return COIN_ETHEREUM;
...
CoinToken.getTokenToMainTypeMap = function(tokenType) {
    if (tokenType === CoinToken.TheDAO) {
        ...
        return COIN_ICONOMI_ETHEREUM;
    } else if (tokenType === CoinToken.Golem) {
        return COIN_GOLEM_ETHEREUM;
    } else if (tokenType === CoinToken.Gnosis) {
        return COIN_GNOSIS_ETHEREUM;
    } else if (tokenType === CoinToken.Singulardtv) {
        return COIN_SINGULARDTV_ETHEREUM;
    } else if (tokenType === CoinToken.Digix) {
        return COIN_DIGIX_ETHEREUM;
    } else if (tokenType === CoinToken.BlockchainCapital) {
        return COIN_BLOCKCHAINCAPITAL_ETHEREUM;
    } else if (tokenType === CoinToken.Civic) {
        return COIN_CIVIC_ETHEREUM;
    } 
}
...
//    console.log("token type for main coin type :: " + coinType);
    ...
    } else if (mainType === COIN_ICONOMI_ETHEREUM) {
        return CoinToken.Iconomi;
    } else if (mainType === COIN_GOLEM_ETHEREUM) {
        return CoinToken.Golem;
    } else if (mainType === COIN_GNOSIS_ETHEREUM) {
        return CoinToken.Gnosis;
    } else if (mainType === COIN_SINGULARDTV_ETHEREUM) {
        return CoinToken.Singulardtv;
    } else if (mainType === COIN_DIGIX_ETHEREUM) {
        return CoinToken.Digix;
    } else if (mainType === COIN_BLOCKCHAINCAPITAL_ETHEREUM) {
        return CoinToken.BlockchainCapital;
    } else if (mainType === COIN_CIVIC_ETHEREUM) {
        return CoinToken.Civic;
    }
}
...
CoinToken.getStaticTokenImplementation = function(tokenType) {
    ...
    if (tokenType === CoinToken.TheDAO) {
        ...
        return CoinTokenIconomiEthereum;
    } else if (tokenType === CoinToken.Golem) {
        return CoinTokenGolemEthereum;
    } else if (tokenType === CoinToken.Gnosis) {
        return CoinTokenGnosisEthereum;
    } else if (tokenType === CoinToken.Singulardtv) {
        return CoinTokenSingulardtvEthereum;
    } else if (tokenType === CoinToken.Digix) {
        return CoinTokenDigixEthereum;
    } else if (tokenType === CoinToken.BlockchainCapital) {
        return CoinTokenBlockchainCapitalEthereum;
    } else if (tokenType === CoinToken.Civic) {
        return CoinTokenCivicEthereum;
    }
}
...
CoinToken.getNewTokenImplementation = function(tokenType) {
    ...
    if (tokenType === CoinToken.TheDAO) {
        ...
        return new CoinTokenIconomiEthereum();
    } else if (tokenType === CoinToken.Gnosis) {
        return new CoinTokenGnosisEthereum();
    } else if (tokenType === CoinToken.Golem) {
        return new CoinTokenGolemEthereum();
    } else if (tokenType === CoinToken.Singulardtv) {
        return new CoinTokenSingulardtvEthereum();
    } else if (tokenType === CoinToken.Digix) {
        return new CoinTokenDigixEthereum();
    } else if (tokenType === CoinToken.BlockchainCapital) {
        return new CoinTokenBlockchainCapitalEthereum();
    } else if (tokenType === CoinToken.Civic) {
        return new CoinTokenCivicEthereum();
    }
}
...
CoinToken.prototype.initialize = function(tokenName, tokenSymbol, tokenCoinType, baseReceiveAddress, coinHolderWallet, gasPrice, gasLimit, storageKey, contractAddress) {
...
CoinToken.prototype.buildERC20EthereumTransactionList = function(ethereumPouch, toAddress, amount_smallUnit, gasPrice, gasLimit, ethereumTXDataPrePendArray, doNotSign) {
    ...
    if(curCoinType === COIN_DIGIX_ETHEREUM) {
        var amountDao = amount_smallUnit / 1000000000;
    } else if (curCoinType === COIN_CIVIC_ETHEREUM) {
        var amountDao = amount_smallUnit / 10000000000;
    } else if(curCoinType === COIN_BLOCKCHAINCAPITAL_ETHEREUM) {
        var toBig = new thirdparty.Big('1000000000000000000')
        var amountSmall = new thirdparty.Big(amount_smallUnit);
        var amountDao = amountSmall.div(toBig);
    } else {
