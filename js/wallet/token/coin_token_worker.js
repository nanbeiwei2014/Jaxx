...
//        this._STATIC_RELAY_URL = "http://api.jaxx.io:80/api/thedao";
        ...
        this._STATIC_RELAY_URL = "http://api.jaxx.io/api/thedao";
        
        this._GATHER_TX = "/balance?addresses=";
        
        this._REQUEST_BATCH_SIZE = 20;
        ...
        this._GATHER_TX = "&action=tokenbalance&contractaddress=0x48c80F1f4D53D5951e5D5438B54Cba84f29F32a5" + "&address=";

        this._GATHER_TX_APPEND = "&tag=latest&apikey=" + HDWalletHelper.apiKeyEtherScan;
        
        this._REQUEST_BATCH_SIZE = 1;
        ...
        // https://api.etherscan.io/api?module=account&action=tokenbalance&contractaddress=0x48c80F1f4D53D5951e5D5438B54Cba84f29F32a5&address=0xddbd2b932c763ba5b1b7ae3b362eac3e8d40121a,0x63a9975ba31b0b9626b34300f7f627147df1f526&tag=latest&apikey=WGWHHAU4F2Y58UW5FQWTUJWSXBNHU7WBSX
    }
//
//    var self = this;
//
...
//    console.log("addressesToCheck :: " + JSON.stringify(addressesToCheck) + " :: " + addressesToCheck.length);
    ...
    requestCounter.batchSize = this._REQUEST_BATCH_SIZE; //@note: for etherscan.io
...
//            var addressParam = batch.join('&addresses%5B%5D=');
            ...
            var requestString = this._STATIC_RELAY_URL + this._GATHER_TX + addressParam + this._GATHER_TX_APPEND;
            // https://api.etherscan.io/api?module=account&tokenname=TheDAO&action=tokenbalance&address=
            
//            var requestString = this._STATIC_RELAY_URL + this._MULTI_BALANCE + "?addresses%5B%5D=" + addressParam;
            // https://api.etherscan.io/api?module=account&tokenname=TheDAO&action=tokenbalance&address=0x25040290d7ed172c74a0ccae37e09b413c26155b&tag=latest&apikey=WGWHHAU4F2Y58UW5FQWTUJWSXBNHU7WBSX // A sample request string
            // @NOTE: Right now the data
...
//            @note: @here: request the account balances for this batch
            RequestSerializer.getJSON(requestString, function(data, success, passthroughParam) {
                // https://api.etherscan.io/api?module=account&tokenname=TheDAO&action=tokenbalance&address=0x25040290d7Ed172C74A0CcAE37E09B413C26155B,0x0d9b0c2e3394e7661b115864e5aD83AadF013552
                // JSON.stringify(passthroughParam)
                // "{"batch":["0x25040290d7ed172c74a0ccae37e09b413c26155b"],"requestCounter":{"numRequests":49,"processedRequestCount":0,"batchUpdateSize":20,"curBatchCount":0,"batchSize":1}}"
...
//                log("updateBalancesTheDAO :: data :: " + JSON.stringify(data));
                if (success === 'success') {
                    ...
                    if (self._tokenCoinType === CoinToken.TheDAO) {
                        if (data){
                            var addressesFromResponse = Object.keys(data);
                            for (var i = 0; i < addressesFromResponse.length; i++){
                                if (Object.keys(self._addressMap).indexOf(addressesFromResponse[i]) === -1) {
                                    self.createNewAddressMapping(addressesFromResponse[i]);
                                }
                                self._addressMap[addressesFromResponse[i]].balance = parseInt(data[addressesFromResponse[i]]);
                            }
                        } else {
                            log("CoinTokenWorker :: updateBalances :: error :: " + data.status);
                        }
                    } else if (self._tokenCoinType === CoinToken.Augur) {
