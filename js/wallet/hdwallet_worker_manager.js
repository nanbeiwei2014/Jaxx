...
HDWalletWorkerManager.prototype.log = function() {
...
//    args.unshift('WorkerLog:')
    ...
    //
    //        args[0] = "%c" + allArguments[0];
    //        args[args.length] = "color: red;";//; background: yellow;";
    //        args.length++;
    //    }
    //    
    //    console.log(args);
...
HDWalletWorkerManager.prototype.initialize = function(coinType, testNet) {
    ...
    if (relayManagerParams.isSupported === true) {
        this.log("[ HDWalletWorkerManager ] :: setup relay manager :: " + this._coinType);
        ...
        this._relayManager.setup(function(resultParams) {
            ...
            self.log("[ HDWalletWorkerManager ] :: RelayTests :: fetchBlockHeights :: " + JSON.stringify(resultParams));
        }); // Setup the relays (Stored in a global so that instance data is not discarded.)
    } else {
        this.log("[ HDWalletWorkerManager ] :: no relay manager :: " + this._coinType);
...
HDWalletWorkerManager.prototype.finishInitialization = function() {
    ...
    this.log("[ HDWalletWorkerManager ] :: init :: " + this._coinType);
    ...
    if (socketUri !== "") {
        ...
        */
        ...
        this._watcherWebSocket.onerror = function(event) {
            self.log("watcher :: " + this._coinType + " :: error :: " + JSON.stringify(event));
        }    
    }
}
...
HDWalletWorkerManager.prototype.setExtendedPublicKeys = function(receivePublicKey, changePublicKey) {
    ...
    this.log(this._coinType + " :: coinNetwork :: " + coinNetwork);
...
//    }
    ...
    if (!this._changeNode || !this._receiveNode) {
        this.log("watcher :: " + this._coinType + " :: checkTransactions :: nodes required");
        return;
    }
        ...
        if (message.data.content.type && message.data.content.type === 'balances') {
            setTimeout(function() {
                if (hdWalletWorkerManager._coinType === COIN_ETHEREUM || hdWalletWorkerManager._coinType === COIN_ETHEREUM_CLASSIC ||
                    ...
                    hdWalletWorkerManager.log(hdWalletWorkerManager._coinWorkerImpl._coinName + " :: restore address map balance refresh");
                    hdWalletWorkerManager._coinWorkerImpl.updateBalances();
                }
            }, 10000);
        }
    } else if (message.data.action === 'refresh') {
        if(typeof log ==='function') log("watcher :: " + hdWalletWorkerManager._coinType + " :: refreshing");
...
//        log('Refreshing...');
        setTimeout(function () {
            setTimeout(function() {
                if (hdWalletWorkerManager._coinType === COIN_ETHEREUM || hdWalletWorkerManager._coinType === COIN_ETHEREUM_CLASSIC ||
                    hdWalletWorkerManager._coinType === COIN_TESTNET_ROOTSTOCK) {
                    hdWalletWorkerManager.log(hdWalletWorkerManager._coinWorkerImpl._coinName + " :: manual refresh balance refresh");
                    hdWalletWorkerManager._coinWorkerImpl.updateBalances();
                }
            }, 10000);
...
setInterval(function() {
    setTimeout(function() {
        if (hdWalletWorkerManager._coinType === COIN_ETHEREUM) {
            hdWalletWorkerManager.log("ethereum :: autorefresh balance refresh");
            hdWalletWorkerManager._coinWorkerImpl.updateBalances();
        }
    }, 10000);
    hdWalletWorkerManager.checkTransactions(HDWalletWorkerManager.getDefaultTransactionRefreshTime()); 
