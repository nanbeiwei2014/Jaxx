...
EthereumWallet.convertWeiToEther = function(wei) {
    ...
    */
    if(wei > 0) {
        var balance = wei / 1000000000000000000; //thirdparty.web3.fromWei(wei, 'ether'); breaking with IOS 10.3
        balance = balance.toString(10);
        //console.log("balance wei to ether", balance);
        if (balance.indexOf('.') == -1) {
            balance += '.0';
        }
        //console.log("Count wei to ether called", count);
        return balance;
    } else { return 0; }
}
...
EthereumWallet.convertEtherToWei = function(ether) {
    ...
    return balance;
    */
    var balance =  ether * 1000000000000000000; //thirdparty.web3.toWei(ether, 'ether');
    balance = balance.toString(10);
    if (balance.indexOf('.') == -1) {
        balance += '.0';
    }
...
EthereumWallet.getAddressFromKey = function(privateKey) {
    ...
    var mockupTxRaw = {
        ...
        gasPrice: EthereumWallet.hexify(thirdparty.web3.toBigNumber(thirdparty.web3.toWei(21, 'shannon')).plus(1000000000).toDigits(1)),
