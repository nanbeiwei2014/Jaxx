...
JaxxUI.prototype.updateAndLoadPrivateKeyList = function(coinType) {
    ...
    if (g_JaxxApp.getDataStoreController().getCryptoControllerByCoinType(coinType).hasIndexes()){
        setTimeout(function ()  {
            self.updatePrivateKeyList(coinType);
        }, 500);
    } else {
        g_JaxxApp.getDataStoreController().getCryptoControllerByCoinType(coinType).restoreHistoryAll(function(){
            setTimeout(function ()  {
                self.updatePrivateKeyList(coinType);
            }, 500);
        });
...
JaxxUI.prototype.updatePrivateKeyList = function(coinType) {
    ...
    for (var i = 0; i < accounts.length; i++) {
        ...
        if (coinType === COIN_ETHEREUM) {
            ...
            var accountCVCRawBalance = wallet.getPouchFold(COIN_CIVIC_ETHEREUM).getDataStorageController().getBalanceByAddress(address);
...
JaxxUI.prototype.getJaxxCoinBulletin = function(callback) {
    ...
    var url = "https://jaxx.io/jaxx-data/jaxx-coin-bulletin.php";
...
JaxxUI.prototype.populateCoinBulletinMenu = function(){
    ...
    $(".bulletinPage .scrollableMenuList").empty();
    for (var i = 0; i < coinAbbreviatedNames.length; i++){
        var coinType = HDWalletHelper.dictCryptoCurrency[coinAbbreviatedNames[i]]["index"];
        $(".bulletinPage .scrollableMenuList").append(g_JaxxApp.getUI().getCoinBulletinMenuRow(coinType));
    }
...
JaxxUI.prototype.getCoinBulletinMenuRow = function(coinType){
    ...
    var htmlRow = '<div class="settingsResetCache expandableText cssExpandableText scriptAction cssOpacity expandableDetailsAncestor cssInitialHeight" specialaction="jaxx_controller.showCoinBulletin" value='+coinAbbreviatedName+'>';
    htmlRow += '<div class="cssBulletinImageWrapper bulletinImageWrapper"><div class="cssImageLogoBanner'+coinAbbreviatedName+' cssCoinSelector coinTypeETH cssHighlighted"></div></div>';
    htmlRow += '<div class="expandableDetailsHeader cssExpandableDetailsHeader">';
    htmlRow += '<div class="optionTrigger cssOptionTrigger">';
    htmlRow += '<div class="optionHeading cssOptionHeading">';
    htmlRow += '<label class="menuCoinBulletinTitle cssMenuCoinBulletinTitle">'+menuTitle+'</label>';
    htmlRow += '</div>';
    htmlRow += '</div>';
    htmlRow += '</div>';
    htmlRow += '<div class="postedDateWrapper"><div class="postedLabel cssPostedLabel">Posted:</div>';
    htmlRow += '<div class="postedDate cssPostedDate">'+ date+'</div></div>';
    htmlRow += '</div> ';
    return htmlRow;
}

JaxxUI.prototype.showBulletinWhenSwitchingToCoin = function(coinType){
    // coinType is the coin that we are switching to.
...
JaxxUI.prototype.getReleaseBulletin = function(callback) {
    var self = this;
    // **** Reminder:  When testing use different endpoint - not the live one ****
    var url = "https://jaxx.io/jaxx-data/jaxx-release-notes.php"
    //    var url = "https://jaxx.io/jaxx-data/jaxx-news-beta.php"
    $.getJSON( url, function( data ) {
        if (data && data[0]) {
...
JaxxUI.prototype.getChangeLog = function(callback) {
    var self = this;
    // **** Reminder:  When testing use different endpoint - not the live one ****
    var url = "https://jaxx.io/jaxx-data/change-log.php";
    //    var url = "https://jaxx.io/jaxx-data/jaxx-news-beta.php"
    $.getJSON( url, function( data ) {
        if (data && data[0]) {
            self._changeLog = self.getChangeLogByPlatform(data[0]);
            callback();
        }
    });
}
...
JaxxUI.prototype.getChangeLogSummary = function(callback) {
    var self = this;
    // **** Reminder:  When testing use different endpoint - not the live one ****
    var url = "https://jaxx.io/jaxx-data/change-log-summary.php";
    //    var url = "https://jaxx.io/jaxx-data/jaxx-news-beta.php"
    $.getJSON( url, function( data ) {
        if (data && data[0]) {
            self._changeLogSummary = self.getChangeLogSummaryByPlatform(data[0]);
            callback();
        }
    });
}
...
JaxxUI.prototype.completeSwitchToCoinType = function(currentCoinType, targetCoinType) {
    ...
    this.showModalForCoinBulletinIfNotHidden(targetCoinType);
...
JaxxUI.prototype.showModalForCoinBulletinIfNotHidden = function(coinType){
    var coinAbbreviatedName = HDWalletPouch.getStaticCoinPouchImplementation(coinType).pouchParameters["coinAbbreviatedName"];
    ...
    if (typeof(bulletinData) !== "undefined" && bulletinData !== null) {
        ...
        if (!g_JaxxApp.getSettings().isCoinBulletinHideOnSelect(coinType, version)) {
            this.showCoinBulletinUsingAbbreviatedName(coinAbbreviatedName);
        }
    }
}
...
JaxxUI.prototype.updateCoinDisplayBalanceInWallet = function(coinType, coinBalance, boolOverrideBalanceLock){
   ...
   // console.log(coinBalance);
    ...
    if (!this._debugLockBalanceUpdate || boolOverrideBalanceLock) {
        ...
        var balTemp=  HDWalletHelper.getCoinDisplayScalar(curCoinType, coinBalance);


        console.log(balTemp);
        var coinDisplayBalance = parseFloat(balTemp + "").toFixed(8);

       // console.warn(coinDisplayBalance, curCoinType, coinBalance);
...
//
   // console.warn(balance[curCoinType]);
   ...
   // console.warn(coinBalance, curCoinType, COIN_UNITLARGE, balance[curCoinType]);

    var coinAbbreviatedName = HDWalletPouch.getStaticCoinPouchImplementation(curCoinType).pouchParameters['coinAbbreviatedName'];
    if(curCoinType === COIN_ICONOMI_ETHEREUM) {
        var coinAbbreviatedICNName = "ICN";
        $('.populateBalanceCoinUnit').text(coinAbbreviatedICNName);
    } else {
