    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (publisher == null) {
                ...
                publisher = (yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)("powershell.exe", [`(Get-PfxCertificate "${cscFile}").Subject`])).trim();
                if (!publisher) {
                    throw new Error("Please specify appx.publisher: Get-PfxCertificate returns empty string");
                }
            }
            const preAppx = _path.join(_this.outDir, `pre-appx-${(0, (_core || _load_core()).getArchSuffix)(arch)}`);
            ...
            yield (0, (_promise || _load_promise()).asyncAll)([function () {
                return (_bluebirdLst2 || _load_bluebirdLst2()).default.map(Object.keys(vendorAssetsForDefaultAssets), function (defaultAsset) {
                    if (!isDefaultAssetIncluded(userAssets, defaultAsset)) {
                        (0, (_fs || _load_fs()).copyFile)(_path.join(vendorPath, "appxAssets", vendorAssetsForDefaultAssets[defaultAsset]), _path.join(preAppx, "assets", defaultAsset));
                    }
                });
            }, function () {
                return _this.writeManifest(_path.join(__dirname, "..", "..", "templates", "appx"), preAppx, arch, publisher, userAssets);
            }, function () {
                return (0, (_fs || _load_fs()).copyDir)(appOutDir, _path.join(preAppx, "app"));
            }]);
            ...
            if (isScaledAssetsProvided(userAssets)) {
                ...
                const makePriPath = _path.join(vendorPath, "windows-10", (_core || _load_core()).Arch[arch], "makepri.exe");
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)(makePriPath, ["createconfig", "/cf", priConfigPath, "/dq", "en-US", "/pv", "10.0.0", "/o"]);
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)(makePriPath, ["new", "/pr", preAppx, "/cf", priConfigPath, "/of", preAppx]);
                makeAppXArgs.push("/l");
            }
            ...
            yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)(_path.join(vendorPath, "windows-10", (_core || _load_core()).Arch[arch], "makeappx.exe"), makeAppXArgs);
            yield packager.sign(destination);
            packager.dispatchArtifactCreated(destination, _this, arch, packager.computeSafeArtifactName("appx"));
        })();
    }
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const manifest = (yield (0, (_fsExtraP || _load_fsExtraP()).readFile)(_path.join(templatePath, "appxmanifest.xml"), "utf8")).replace(/\$\{([a-zA-Z0-9]+)\}/g, function (match, p1) {
                switch (p1) {
                    ...
                    case "arch":
                        return arch === (_core || _load_core()).Arch.ia32 ? "x86" : "x64";
