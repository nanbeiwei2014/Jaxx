...
function computeArchToTargetNamesMap(raw, options, platform) {
    ...
    const defaultArchs = raw.size === 0 ? [platform === (_core || _load_core()).Platform.MAC ? "x64" : process.arch] : Array.from(raw.keys()).map(it => (_core || _load_core()).Arch[it]);
    ...
    for (const target of (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(options.target).map(it => typeof it === "string" ? { target: it } : it)) {
        ...
        for (const arch of archs == null ? defaultArchs : (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(archs)) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).addValue)(result, (0, (_core || _load_core()).archFromString)(arch), name);
        }
    }
    if (result.size === 0) {
        for (const arch of defaultArchs) {
            result.set((0, (_core || _load_core()).archFromString)(arch), []);
        }
    }
    return result;
}
